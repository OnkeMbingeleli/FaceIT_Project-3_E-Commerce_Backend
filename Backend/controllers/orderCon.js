import { postorderDb, getordersDb, getorderByIdDb, patchorderDb, deleteorderDb } from "../models/orderDb.js";

export const postorderCon = async (req, res) => {
  try {
    const { sub_id, amount, order_status } = req.body;
    const data = await postorderDb({ sub_id, amount, order_status });
...
export const getordersCon = async (_req, res) => {
  try {
